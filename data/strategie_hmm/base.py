...
from numba import njit # Pour accélerer les algo

import numpy as np
from scipy.special import logsumexp
from sklearn.base import BaseEstimator
import pprint  # à ajouter pour la solution propre
...
###nFonction d'opti Viterbi
#from numba import njit
import numpy as np

@njit
def do_viterbi_pass(framelogprob, log_startprob, log_transmat):
    n_samples, n_components = framelogprob.shape
    viterbi_lattice = np.zeros((n_samples, n_components))
    backpointer = np.zeros((n_samples, n_components), dtype=np.int32)
    ...
    for t in range(1, n_samples):
        for j in range(n_components):
            ...
            for i in range(n_components):
                val = viterbi_lattice[t - 1, i] + log_transmat[i, j]
                if val > max_val:
                    max_val = val
                    arg_max = i
            viterbi_lattice[t, j] = max_val + framelogprob[t, j]
    ...
    last_state = 0
    for j in range(n_components):
        if viterbi_lattice[-1, j] > logprob:
            logprob = viterbi_lattice[-1, j]
            last_state = j
...
### Fonction d'opti Forward
@njit
def do_forward_pass(framelogprob, log_startprob, log_transmat):
    n_samples, n_components = framelogprob.shape
    ...
    for t in range(1, n_samples):
        for j in range(n_components):
            tmp = -1e300  # simule -inf
            for i in range(n_components):
                tmp = np.logaddexp(tmp, fwdlattice[t - 1, i] + log_transmat[i, j])
            fwdlattice[t, j] = tmp + framelogprob[t, j]

    logprob = -1e300
    for j in range(n_components):
        logprob = np.logaddexp(logprob, fwdlattice[-1, j])

    return logprob, fwdlattice

### Fonction d'opti Backward
@njit
def do_backward_pass(framelogprob, log_transmat):
    n_samples, n_components = framelogprob.shape
    ...
    for t in range(n_samples - 2, -1, -1):
        for i in range(n_components):
            s = -1e300  # simule -inf
            for j in range(n_components):
                s = np.logaddexp(s, log_transmat[i, j] + framelogprob[t + 1, j] + bwdlattice[t + 1, j])
            bwdlattice[t, i] = s
...
class ConvergenceMonitor(object):
    ...
    def report(self, logprob):
        ...
        # 	print('rest:', self.history[1] - self.history[0])
        self.iter += 1
    ...
    def converged(self):
        ...
        return (self.iter == self.n_iter or
                (len(self.history) == 2 and
                 np.abs(self.history[1] - self.history[0]) < self.tol))
...
class _BaseHMM(BaseEstimator):
    ...
    def __init__(self, n_components=1,
                 ...
                 n_iter=30, tol=1e-3, verbose=False,
    ...
    #def _do_viterbi_pass(self, framelogprob):
     #   n_samples, n_components = framelogprob.shape
     #   viterbi_lattice = np.zeros((n_samples, n_components))
     #   backpointer = np.zeros((n_samples, n_components), dtype=int)

     #   viterbi_lattice[0] = np.log(self.startprob_) + framelogprob[0]

       # for t in range(1, n_samples):
       #     for j in range(n_components):
        #        transition_probs = viterbi_lattice[t - 1] + np.log(self.transmat_[:, j])
         #       backpointer[t, j] = np.argmax(transition_probs)
          #      viterbi_lattice[t, j] = np.max(transition_probs) + framelogprob[t, j]
    ...
    #def _do_forward_pass(self, framelogprob):
     #   n_samples, n_components = framelogprob.shape
      #  fwdlattice = np.zeros((n_samples, n_components))
       # fwdlattice[0] = np.log(self.startprob_) + framelogprob[0]

        #for t in range(1, n_samples):
         #   for j in range(n_components):
          #      fwdlattice[t, j] = logsumexp(fwdlattice[t - 1] + np.log(self.transmat_[:, j])) + framelogprob[t, j]

        #logprob = logsumexp(fwdlattice[-1])
        #return logprob, fwdlattice

    def _do_forward_pass(self, framelogprob):
        return do_forward_pass(
        framelogprob,
        np.log(self.startprob_ + 1e-10),
        np.log(self.transmat_)
    )
   ...
   # def _do_backward_pass(self, framelogprob):
    ...
    def _do_backward_pass(self, framelogprob):
        return do_backward_pass(
        framelogprob,
        np.log(self.transmat_ + 1e-10)
        )
   ...
   # def _do_backward_pass(self, framelogprob):
     #   n_samples, n_components = framelogprob.shape
      #  bwdlattice = np.zeros((n_samples, n_components))
       ...
       #     for i in range(n_components):
       #         bwdlattice[t, i] = logsumexp(
       #             np.log(self.transmat_[i]) + framelogprob[t + 1] + bwdlattice[t + 1]
       #         )
       # return bwdlattice
    ...
    def _accumulate_sufficient_statistics(self, stats, X, framelogprob,
        ...
        if 't' in self.params:
            ...
            for t in range(n_samples - 1):
                for i in range(n_components):
                    for j in range(n_components):
                        lneta[t, i, j] = (
                            fwdlattice[t, i] +
                            np.log(self.transmat_[i, j]) +
                            framelogprob[t + 1, j] +
                            bwdlattice[t + 1, j]
                        )
            # Normalisation log-sum-exp pour éviter les underflows numériques
            lneta -= logsumexp(lneta, axis=(1, 2), keepdims=True)
            ...
            stats['eta_tij'] = np.exp(lneta) #without axis? #this is the only line modified by us
