...
from numba import njit # Pour accélerer les focntions
...
def log_multivariate_normal_density(X, means, covars, covariance_type):
    ...
    for c in range(n_components):
        if covariance_type == 'diag':
            cov = np.diag(covars[c])
        elif covariance_type == 'full':
            cov = covars[c]
        else:
            raise NotImplementedError("Only 'diag' and 'full' are supported here.")

        mvn = multivariate_normal(mean=means[c], cov=cov, allow_singular=True)
        log_prob[:, c] = mvn.logpdf(X)

    return log_prob

def distribute_covar_matrix_to_match_covariance_type(
    covar, covariance_type, n_components
):
    """Create a set of covariance matrices from a given template."""
    if covariance_type == "full":
        return np.tile(covar, (n_components, 1, 1))
    elif covariance_type == "tied":
        return covar
    elif covariance_type == "diag":
        return np.tile(np.diag(covar)[np.newaxis, :], (n_components, 1))
    elif covariance_type == "spherical":
        return np.tile(np.mean(np.diag(covar)), (n_components, 1))
    else:
        raise ValueError("Invalid covariance_type")

def _validate_covars(covars, covariance_type, n_components):
    """Validate the shape of covariance matrices."""
    covars = np.asarray(covars)
    if covariance_type == 'full':
        if covars.shape != (n_components, covars.shape[1], covars.shape[2]):
            raise ValueError("Covariance matrix shape does not match 'full'")
    elif covariance_type == 'tied':
        if covars.ndim != 2 or covars.shape[0] != covars.shape[1]:
            raise ValueError("Covariance matrix shape does not match 'tied'")
    elif covariance_type == 'diag':
        if covars.shape != (n_components, covars.shape[1]):
            raise ValueError("Covariance matrix shape does not match 'diag'")
    elif covariance_type == 'spherical':
        if covars.shape != (n_components, 1):
            raise ValueError("Covariance matrix shape does not match 'spherical'")
    else:
        raise ValueError("Invalid covariance type")

def sample_gaussian(mean, covar, covariance_type, random_state=None):
    """Draw a sample from a Gaussian distribution."""
    rng = check_random_state(random_state)
    if covariance_type == "full":
        return rng.multivariate_normal(mean, covar)
    elif covariance_type == "diag":
        return mean + np.sqrt(covar) * rng.randn(len(mean))
    elif covariance_type == "spherical":
        return mean + np.sqrt(covar[0]) * rng.randn(len(mean))
    elif covariance_type == "tied":
        return rng.multivariate_normal(mean, covar)
    else:
...
def iter_from_X_lengths(X, lengths):
    """Generator that yields slice indices for each sequence in X based on lengths."""
    if lengths is None:
        yield 0, len(X)
    else:
        start = 0
...
#@njit
#def compute_FS_ESTEP_nb(X, gamma, means, covars, epsilon, tau, rho):
#    I = gamma.shape[1]
 #   L = X.shape[1]
 #   T = X.shape[0]
  #  uilt = np.zeros((I, L, T))
   ...
   # for i in range(I):
    ...
    #        for t in range(T):
     #           x = X[t, l]
      #          mu = means[i, l]
       #         sigma2 = covars[i, l]
        #        eilt = rho[l] * (1.0 / np.sqrt(2 * np.pi * sigma2)) * np.exp(-0.5 * ((x - mu) ** 2) / sigma2)
         #       hilt = (1.0 - rho[l]) * (1.0 / np.sqrt(2 * np.pi * tau[l] ** 2)) * np.exp(-0.5 * ((x - epsilon[l]) ** 2) / tau[l] ** 2)
          #      gilt = eilt + hilt
           #     uilt[i, l, t] = gamma[t, i] * eilt / gilt
            #    vilt[i, l, t] = gamma[t, i] - uilt[i, l, t]
...
class GaussianHMM(_BaseHMM):
    ...
    # 🟢 Cette ligne DOIT être placée ici (hors des fonctions)
    covars_ = property(_get_covars, _set_covars)
...
class GaussianFSHMM(GaussianHMM):
   ...
   # self.uilt, self.vilt = compute_FS_ESTEP_nb(
   #      X[i:j], posteriors, self.means_, self._covars_,
    #    self.epsilon_, self.tau_, self.rho_
    #)
